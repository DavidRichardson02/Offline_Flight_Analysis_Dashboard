...
arguments
    ...
    options.RenderDiagnosticFigures (1,1) logical = true
end
...
results = caelum.analyzeLog(inputInfo.filename, ...
    MakePlots=caseDef.makePlots && options.RenderDiagnosticFigures, ...
    ReplayEstimator=true, ...
    Config=cfg, ...
    MakeDashboard=caseDef.makeDashboard && options.RenderDiagnosticFigures, ...
...
artifactCaseDef = caseDef;
artifactCaseDef.makeDashboard = caseDef.makeDashboard && options.RenderDiagnosticFigures;
exportInfo = caelum.exportFigures(results, exportDir);

artifactRows = localArtifactRows(artifactCaseDef, modeDef, exportInfo);
...
switch caseDef.sourceType
    ...
    case "existing"
        inputInfo.filename = localResolveExistingCasePath(caseDef.sourcePath);
    otherwise
...
function filename = localResolveExistingCasePath(sourcePath)
sourcePath = string(sourcePath);
candidates = [ ...
    sourcePath; ...
    string(fullfile(fileparts(mfilename('fullpath')), sourcePath)); ...
    string(fullfile(fileparts(mfilename('fullpath')), "Flight Data", sourcePath)); ...
    string(fullfile(fileparts(mfilename('fullpath')), "CaelumSufflamen", "validation", "flight data", sourcePath))];
...
error('validate_vertical_replay_stack:ExistingCaseMissing', ...
    'Existing validation case file was not found: %s', sourcePath);
end
...
missingLatest = setdiff(requiredFields, actualFields);
...
compatRequired = [ ...
    ...
    "P00","P01","P10","P11"];
missingCompat = setdiff(compatRequired, actualFields);

isLatestFirmwareLog = isempty(missingLatest);
isDashboardCompatibleGeneratedLog = isempty(missingCompat);
passed = isLatestFirmwareLog || isDashboardCompatibleGeneratedLog;

if isLatestFirmwareLog
    actual = "latest_firmware_sd_schema";
    notes = "Generated input matches the latest checked-in firmware SD logger schema.";
elseif isDashboardCompatibleGeneratedLog
    actual = "dashboard_compatible_generated_schema";
    notes = "Generated replay input uses the full dashboard compatibility schema; latest phase/policy fields are not required for synthetic replay cases.";
else
    actual = localMissingString(missingCompat);
    notes = "Generated input is missing fields required by both the latest firmware schema and the dashboard compatibility schema.";
end

rows = [rows; localValidationRow(caseDef.name, modeDef.name, ...
    "firmware_input", "required_sdlog_fields_present", passed, "gate", ...
    actual, "latest_firmware_or_dashboard_compatible", NaN, notes)]; %#ok<AGROW>
end
