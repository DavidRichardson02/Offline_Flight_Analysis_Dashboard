...
[Tbase, truth] = caelum.generateTruthAwareCaelumLog(filename, ...
    ...
    launchDelay_s=opts.launchDelay_s, ...
    boostDuration_s=opts.boostDuration_s, ...
    tailoffDuration_s=opts.tailoffDuration_s, ...
    boostAccel_mps2=opts.boostAccel_mps2, ...
    tailoffStartAccel_mps2=opts.tailoffStartAccel_mps2, ...
    dragCoeff=opts.dragCoeff, ...
    baroAltNoise_m=opts.baroAltNoise_m, ...
    accelNoise_mps2=opts.accelNoise_mps2, ...
    gyroNoise_rps=opts.gyroNoise_rps, ...
    lisNoise_mps2=opts.lisNoise_mps2, ...
    accelBiasXYZ=opts.accelBiasXYZ, ...
    gyroBiasXYZ=opts.gyroBiasXYZ, ...
    lisBiasXYZ=opts.lisBiasXYZ, ...
