...
arguments
    ...
    options.MakeAuxiliaryDashboard (1,1) logical = false
...
if options.MakeDashboard
    ...
    if options.MakeAuxiliaryDashboard
        auxiliaryDashboardError = [];
        try
            auxiliaryDashboardFigure = caelum.plotAuxiliaryDashboard(clean, events, replay, cfg, Est3D=est3d);
        catch ME
            auxiliaryDashboardFigure = gobjects(0, 1);
            auxiliaryDashboardError = ME;
            warning('caelum:analyzeLog:AuxiliaryDashboardFailed', ...
                'Auxiliary dashboard rendering failed: %s', ME.message);
        end
    else
        auxiliaryDashboardFigure = gobjects(0, 1);
        auxiliaryDashboardError = [];
...
results.auxiliaryDashboardFigure = auxiliaryDashboardFigure;
if isempty(auxiliaryDashboardError)
    results.auxiliaryDashboardErrorIdentifier = "";
    results.auxiliaryDashboardErrorMessage = "";
else
    results.auxiliaryDashboardErrorIdentifier = string(auxiliaryDashboardError.identifier);
    results.auxiliaryDashboardErrorMessage = string(auxiliaryDashboardError.message);
end
